...
DOC_ROOT="${REPO_ROOT}/docs/experiments/2026-04-15/2026-04-15_16-10_v01_wo-semantic-mask-ablation-plan"
RUNLOG_FILE="${DOC_ROOT}/RUNLOG.md"
SUDO_PASSWORD="${SUDO_PASSWORD:-a}"

RESULT_ROOT="/workspace/tmp/2026-04-15/2026-04-15-cifar10-wo-semantic-mask-ablation"
HOST_RESULT_ROOT="${REPO_ROOT}/tmp/2026-04-15/2026-04-15-cifar10-wo-semantic-mask-ablation"

log_to() {
  local ts
  ts="$(date '+%Y-%m-%d %H:%M:%S %Z')"
  printf -- "- [%s] %s\n" "${ts}" "$1" | tee -a "${RUNLOG_FILE}"
}
...
run_one() {
  ...
  label="cifar10_${channel}_wo_semantic_mask_seed_${seed}"
  expected_npz="$(npz_path "${channel}" "${beta}" "${film_max}" "${film_min}" "${seed}")"

  if wait_for_artifact "${expected_npz}" 2 1; then
    log_to "SKIP ${label} (existing artifact: ${expected_npz})"
    return 0
  fi
  ...
  if run_exp \
    ...
    --pruning_threshold 0.0 --film_max_t "${film_max}" --film_min_t "${film_min}" \
  ...
  if wait_for_artifact "${expected_npz}" 20 1; then
    log_to "END ${label} (artifact exists despite non-zero exit: ${expected_npz})"
    return 0
  fi

  log_to "FAIL ${label} (missing artifact after non-zero exit)"
  return 1
}

main() {
  printf '%s\n' "${SUDO_PASSWORD}" | sudo -S mkdir -p "${HOST_RESULT_ROOT}" "${DOC_ROOT}"
  : > "${RUNLOG_FILE}"
  log_to "w/o semantic mask ablation 시작"
  ...
  log_to "w/o semantic mask ablation 완료"
}
